...
<template>
  <div v-bind:id="wallId" class="img-wall">\
		  <row v-for="row in rows" v-bind:thumbnails="thumbnailsByRow(row)"></row>\
          ...
          column: {
            ...
            methods: {
              ...
              getImageUrl: function (name) {
                var lang = TM.lang.replace('-', '_').toLowerCase();
                var imgName = parseInt(name, 10).toString();
                return TM.urlTheme + TM.cdnUrl + '/dist/assets/img/' + lang + '/leaderboard-games/' + imgName + '.png';
              },

              onClick: function () {
                this.$parent.$parent.$emit('zoomIn', this.getImageUrl(this.thumbnail.name), this.thumbnail); //notify to img-wall
              }
            }
          }
        }
    ...
    methods: {
      ...
      init: function init() {
        ...
        var totalThumbnails = this.thumbnails.length,
          minSqrt = isNaN(totalThumbnails) ? 0 : Math.floor(Math.sqrt(totalThumbnails));

        if (totalThumbnails <= this.maxCells * this.maxRows) {
          if (true /*因為landscape排版比較優越, 所以就用這個*/ /*this.layout == 'Landscape'*/) {
      ...
      resize: function () {
        ...
        if (this.$el.offsetWidth > 0 && this.$el.offsetHeight > 0) {
          this.cellWidth = Math.floor(Math.min((this.$el.offsetHeight / Math.min(this.rows, this.maxRows)), (this.$el.offsetWidth / Math.min(this.cells, this.maxCells)))) + 'px';
        } else {
      ...
      thumbnailsByRow: function (row) {
        return this.thumbnails.slice((row - 1) * this.cells, Math.min(row * this.cells, this.thumbnails.length));
      },
...
.img-wall__popup {
  ...
  justify-content: center;
  background: #f0d47e;
  background: -webkit-linear-gradient(rgba(240, 212, 126, 0.7), rgba(206, 138, 98, 0.7));
  background: -o-linear-gradient(rgba(240, 212, 126, 0.7), rgba(206, 138, 98, 0.7));
  background: -moz-linear-gradient(rgba(240, 212, 126, 0.7), rgba(206, 138, 98, 0.7));
  background: linear-gradient(rgba(240, 212, 126, 0.7), rgba(206, 138, 98, 0.7));
}
